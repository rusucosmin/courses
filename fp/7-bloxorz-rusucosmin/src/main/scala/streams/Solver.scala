...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
      ...
      neighbors filter ((b, l) => !(explored contains b))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      ...
      case (block, history) #:: rest => {
        val nxt = for {
          (nxtBlock, nxtHistory) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
        } yield (nxtBlock, nxtHistory)
        nxt ++ from(rest ++ nxt, explored + block)
      }
    }
  }
